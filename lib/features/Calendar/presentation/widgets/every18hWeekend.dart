...
class Every18hWeekend extends StatefulWidget {
  final User user;
  final String token;
  ...
  const Every18hWeekend({Key key, this.user, this.token, this.listProp, this.propertiesModel, this.calendarMonth, this.calendar}) : super(key: key);
  @override
  _Every18hWeekendState createState() => _Every18hWeekendState();
}

class _Every18hWeekendState extends State<Every18hWeekend> {
      ...
      child: Stack(
        ...
        children: [
          Container(
            height: 40,
            width: 100,
            decoration: (index == indexSelected)?BoxDecoration(
              borderRadius: BorderRadius.circular(20.0),
          ...
          if(index == indexSelected)Positioned(
            right: -4,
            top: -8,
            child: Icon(
              Icons.check_circle,
  ...
  String dayFormate(CalendarMonth month,int index){
    return textDisplay(month)+" / "+month.monthName+" "+month.year.toString()+" / "+month.listTimes[index].title;
